...
No hace falta tener Node ni git instalado: el propio bootstrap los resuelve.
...
2. clona el repo (o baja el zip si no hay git) a `~/opencode-telegram-group-topics-bot`;
3. ejecuta el setup universal.

## Setup universal (ya con Node / ya clonado)

```bash
node scripts/setup.mjs            # asistente interactivo
# o con flags (silencioso):
node scripts/setup.mjs --token 123456:ABC --user-id 123456
```

Flags: `--token`, `--user-id`, `--provider` (default `opencode`), `--model`
(default `big-pickle`), `--api-url` (default `http://localhost:4096`),
`--server-user` (default `opencode`), `--server-password`, `--locale`,
`--no-start`, `--no-autostart`, `--no-build`, `--yes`, `--dry-run`, `--help`.

Qué hace (en orden):

1. escribe `.env` (fusionando lo que ya exista);
2. `npm install` (solo si falta `node_modules`) y `npm run build`;
3. arranca el bot oculto en segundo plano;
4. instala el autostart con el login, según plataforma:
   ...
   - **macOS**: agente launchd (`~/Library/LaunchAgents/...plist`).
...
1. Crear el grupo en Telegram con **Topics** habilitado, agregar el bot como admin
   con permiso **Manage Topics** y (en @BotFather) `/setprivacy` → **Disable**.
2. `opencode serve` corriendo (o accesible) en `OPENCODE_API_URL`.
3. Correr el bootstrap/setup, dar token + tu user ID.
4. Probar: DM `/start`; en General `/status` (server healthy) → `/projects` → `/new`.